...
class TestEbikeE2EScenario:
    ...
    def test_step12a_gate_1_1_check(self, mock_sb, client):
        ...
        resp = client.get("/api/v1/gates/1.1/check", params={"project_id": PROJECT_ID})

        assert resp.status_code == 200
        gate = resp.json()

        assert gate["gate_id"] == "1.1"
    ...
    def test_step12b_gate_1_2_check(self, mock_sb, client):
        ...
        resp = client.get("/api/v1/gates/1.2/check", params={"project_id": PROJECT_ID})

        assert resp.status_code == 200
        gate = resp.json()

        assert gate["gate_id"] == "1.2"
        assert gate["passed"] is True
        assert len(gate["failed_reasons"]) == 0
        assert len(gate["checklist_items"]) == 3
