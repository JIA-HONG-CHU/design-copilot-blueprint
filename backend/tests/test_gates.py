...
# ---------------------------------------------------------------------------
# Gate 1.2 — Assumptions + High-risk + Contradictions
# ---------------------------------------------------------------------------
...
class TestGate12:
    ...
    def test_gate_12_pass(self, mock_sb, client):
        ...
        resp = client.get("/api/v1/gates/1.2/check", params={"project_id": "p1"})
        assert resp.status_code == 200
        body = resp.json()
        assert body["passed"] is True
        assert len(body["checklist_items"]) == 3
...
# ---------------------------------------------------------------------------
# Gate 2.2 — Alternatives with MUST pass
# ---------------------------------------------------------------------------
...
class TestGate22:
    ...
    def test_gate_22_pass(self, mock_sb, client):
        ...
        resp = client.get("/api/v1/gates/2.2/check", params={"project_id": "p1"})
        assert resp.status_code == 200
        body = resp.json()
        assert body["passed"] is True
...
class TestGateInvalid:
    ...
    def test_missing_project_id_returns_422(self, client):
        resp = client.get("/api/v1/gates/1.1/check")
        assert resp.status_code == 422


# ---------------------------------------------------------------------------
# Gate PG3 — always requires manual confirmation
# ---------------------------------------------------------------------------
...
class TestGatePG3:
    ...
    def test_pg3_always_fails(self, mock_sb, client):
        ...
        resp = client.get("/api/v1/gates/PG3/check", params={"project_id": "p1"})
        assert resp.status_code == 200
        body = resp.json()
        assert body["passed"] is False
        assert len(body["failed_reasons"]) > 0
