...
> **v3 (2026-03-26)**：從使用者流程出發重新設計，對稱雙軌 + 節點互動定義。
> 對齊 v8 方法論：產出與選擇分離、SCAMPER 創意工具、3 層子系統、假設可證偽性。
...
│  │  │ [AI 生成] [查看結果]  ││  │  │ [啟動 Phase A] [結果] ││        │
│  │  └──────────────────────┘│  │  └──────────────────────┘│        │
│  │          ↓               │  │          ↓               │        │
│  │  ┌ R2 TRIZ 解矛盾 ─────┐│  │  ┌ F2 子系統定義 ───────┐│        │
│  │  │ [啟動 Phase A] [結果] ││  │  │ [AI 建議] [手動新增]  ││        │
│  │  └──────────────────────┘│  │  └──────────────────────┘│        │
│  │          ↓               │  │          ↓               │        │
│  │  ┌ R3 子系統定義 ───────┐│  │  ┌ F3 SCAMPER 變形 ────┐│        │
│  │  │ [AI 建議] [手動新增]  ││  │  │ [AI 生成] [採用/跳過] ││        │
│  │  └──────────────────────┘│  │  └──────────────────────┘│        │
│  │          ↓               │  │          ↓               │        │
│  │  ┌ R4 SCAMPER 變形 ────┐│  │  ┌ 正向候選池 ──────────┐│        │
│  │  │ [AI 生成] [採用/跳過] ││  │  │ TC候選 PC候選 SF候選  ││        │
│  │  └──────────────────────┘│  │  │ SCAMPER候選          ││        │
│  │          ↓               │  │  │ 共 N 個 pending      ││        │
│  │  ┌ 反向候選池 ──────────┐│  │  └──────────────────────┘│        │
│  │  │ AA晉升 TC PC SF SCAM ││  │                          │        │
│  │  │ 共 N 個 pending      ││  │  進度：F2 進行中          │        │
│  │  └──────────────────────┘│  │  收斂：Phase A 85% ●     │        │
│  │                          │  │                          │        │
│  │  進度：R3 進行中          │  │                          │        │
│  │  收斂：Phase A 72% ●     │  │                          │        │
...
│  │                                                                │ │
│  │  ┌────────┐ ┌────────┐ ┌────────┐ ┌────────┐ ┌────────┐    │ │
│  │  │ R-AA晉升│ │ R-TC   │ │ F-TC   │ │ F-PC   │ │ F-SCAM │    │ │
│  │  │ 信心:65 │ │ 信心:78│ │ 信心:82│ │ 信心:71│ │ 信心:68│    │ │
│  │  │ [adopt] │ │ [adopt]│ │[adopt] │ │ [skip] │ │ [skip] │    │ │
│  │  └────────┘ └────────┘ └────────┘ └────────┘ └────────┘    │ │
│  │                                                                │ │
...
**對稱原則**：兩條路徑的步驟數量、卡片大小、按鈕位置完全對稱。反向多一步（Anti-Anchor），所以反向有 R1-R4，正向有 F1-F3 + 候選池。視覺上兩欄等高，正向底部留白或顯示候選池摘要。
...
| 原則 | 說明 |
|------|------|
| **等高等寬** | 兩條路徑的卡片區域等高，短的那條底部顯示候選池摘要填充空間 |
| **對稱步驟名** | R1↔（無對應）、R2↔F1、R3↔F2、R4↔F3 |
| **統一按鈕風格** | 兩條路徑的 AI 按鈕、狀態切換、確認按鈕完全一致 |
| **路徑色彩** | 反向 = amber 系（⚡暖色 = 發散）、正向 = blue 系（🎯冷色 = 收斂） |
| **候選池對齊** | 兩個候選池在視覺上底部對齊，方案卡片格式一致 |
