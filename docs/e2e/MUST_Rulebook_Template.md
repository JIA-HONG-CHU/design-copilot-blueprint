...
## 說明
此模板用於定義可機器執行的 MUST 條件，作為 RD Design Copilot 流程中 **Step 5e (MUST 快篩)** 及 **Step 6 / Gate C (CAD Gate)** 的依據。每條 MUST 規則都應詳細定義其輸入、判定邏輯、所需證據類型及不通過時的處理方式。

> **分層證據要求**：同一條 MUST 規則在不同階段有不同的證據門檻。Step 5e 階段尚無 CAD/仿真，允許以 E0-E1 等級（工程估算 / Expert Judgement）進行初篩；Step 6 / Gate C 階段則要求 E2+ 等級（仿真 / Bench Test）的正式驗證。
...
| **Step 5e 證據要求** | Step 5e 快篩階段所需的最低證據類型（E0-E1 等級）。 | Enum |
| **Step 6 / Gate C 證據要求** | Step 6 設計審查 / Gate C 通過所需的證據類型（E2+ 等級）。 | Enum |
...
| MUST ID | 條件描述 | Input 欄位 (Schema Path) | 判定邏輯 / 公式 | Step 5e 證據要求 (E0-E1) | Step 6 / Gate C 證據要求 (E2+) | Fail 處置 |
|---|---|---|---|---|---|---|
| **M1** | **空間約束**：方案概念必須能置入目標空間包絡內 | `Concept Route.Interface.Envelope.Description`, `Constraint.Hard_Constraints.Volume` | 幾何包絡估算 (Step 5e) 或 `CAD_Model.Interference_Check()` (Step 6) | `Envelope Description` + `Expert Judgement` | `CAD Model (Verified)` + 干涉檢查報告 | `淘汰` |
...
| **M5** | **供應可行性**：方案概念的關鍵零組件必須具備基本的供應韌性 | `Concept Route.Key_Components.Supplier_Audit_ID` | `Approved_Suppliers.Count >= 2` (Gate C) 或初步確認可行 (Step 5e) | `SC Initial Inquiry` / `Expert Judgement` | `Supplier Audit Report (Reviewed)` / `SC Response (Verified)` | `淘汰` |
...
### Step 6 / Gate C (CAD Gate) — 精篩
- **目的**：基於 MVP CAD 和初步仿真，嚴格驗證 MUST 條件。
- **證據等級**：E2 (仿真/Bench Test) 以上。
- **判定原則**：所有 MUST 都必須有 E2+ 證據支撐，否則觸發 Step 6e 證據補齊迴圈。
...
**版本**: v1.1
**最後更新**: 2026-03-12
**適用範圍**: RD Design Copilot Step 5e (MUST 快篩) 及 Step 6 / Gate C (CAD Gate)
