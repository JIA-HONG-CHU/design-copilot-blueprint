...
| 目標層級 | 目標描述 | 對應痛點 |
|---------|---------|---------|
| **O1** | 擴大設計可能性空間，打破路徑依賴 | 經驗鎖定、腦內庫存有限 |
| **O2** | 使未知可見、可追蹤 | 假設隱藏 |
| **O3** | 前置風險驗證，證據驅動決策 | 風險後置、證據缺口不可見 |
...
| **KPI-4** | 方案探索數量 | 1-2 條 | ≥3 條 (含 Anti-Anchor) | 方案集合統計 |
| **KPI-5** | 決策可追溯率 | 低 | 100% | KT 記錄 + Evidence Matrix 完整度 |
| **KPI-6** | 用戶採納率 | N/A | ≥70% | 使用率統計 |
| **KPI-7** | 證據缺口識別率 | N/A | ≥90% | Evidence Matrix 覆蓋率 |
...
| 文件名稱 | 說明 |
|---------|------|
| RD_Design_Copilot_整合流程.md | 方法論詳細說明 (v1.4) |
| RD_Design_Copilot_State_Machine.md | 雙層狀態機與 R&R (v1.2) |
| AI_Agent_Architecture.md | Multi-Agent 架構 (v1.1) |
| MUST_Rulebook_Template.md | 可機器執行 MUST 規則模板 (v1.1) |
| Evidence_Matrix_Risk_Register_Template.md | Evidence Matrix 與 Risk Register 模板 (v1.1) |
| Pre_CAD_Review_Template.md | Pre-CAD 審查報告模板 (v1.1) |
...
| 2.1 | 2026-03-12 | 新增 F1.7 多模態素材解讀與結構化提取 (P0)；F2.3 AutoTRIZ 增加矛盾收斂圖（Fatal/Major/Minor 分級，完全收斂）；NFR-13 擴展為用戶端上傳；AI 角色邊界更新 | [作者] |
