...
> - §6.2：決策樹從「互斥分類」改為「預設三層分析 + L2 觸發條件樹」
> - §6.6：刪除「三路徑不能合併」論述；補上 ARIZ 精神的 deepen_link 設計
> - §6.7 新增：TC → PC 深挖契約
> - §7.0 / §7.6 新增：`solve_triz_layered` orchestrator 與分層時序圖
> - §10：F1 → F2 hand-off 升級為 `LayeredTrizSolution[]`
> - §12.1：新增「L2 觸發門檻誤判」風險
...
**v1.1 關鍵轉變**：TC / PC / SF 不再是互斥的分類標籤，而是**同一矛盾的三層視角**。TC 是現象、PC 是核心、SF 是結構 — 這不是選擇題，是診斷報告的三層。完整論述見 `TRIZ_Layered_DrillDown_Optimization.md`。
...
> **v1.1 重寫說明**：舊版本用決策樹把矛盾互斥分類為 TC 或 PC 或 SF，這是 category error —— 把「分析視角選擇」誤編成「矛盾類型單選題」。TC / PC / SF 在 TRIZ 經典理論中是**同一矛盾的三層視角**（現象 / 本質 / 結構），應以 drill-down 方式遞進使用，不是三選一。完整診斷見 `TRIZ_Layered_DrillDown_Optimization.md` §2。
...
> **v1.1 重要變更**：F1 對外的主入口從 `solve_triz` dispatcher 升級為 `solve_triz_layered` orchestrator。原 dispatcher 與三條 `_solve_*` 保留為**底層 primitive**，不再由 UI 直接呼叫。新時序見 §7.0。
...
| `docs/diagrams/triz-to-scamper-flow.md` v11 | 本文件是該流程圖中 F1 節點的 SA 視角文字化；v11 的分層化變更與本文 v1.1 同步 |
...
| `TRIZ_Layered_DrillDown_Optimization.md` v1.0 | 本文件 v1.1 的方法論依據（蘇格拉底診斷與分層架構提案） |

---

**主要實作檔案索引**：
- `backend/app/agents/triz_solver.py` (solve_triz dispatcher + 三條 _solve_*；v1.1 將新增 `solve_triz_layered` orchestrator)
