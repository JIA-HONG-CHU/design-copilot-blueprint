...
| 任務 ID | 工作項 | 交付物 / 完成準則 | 依賴 | 狀態 |
|---------|--------|-------------------|------|------|
| 10.1 | **F3 SCAMPER** 讀取已確認之結構化契約（UC6）；Frozen 後解鎖規則（§8.1） | 契約變更 invalidate 策略有 log 或版本號 | 7.7 | 🔄 Wave 6 進行中（ScamperRequest 加 interface_contracts + contracts_hash；FE 漂移 banner） |
