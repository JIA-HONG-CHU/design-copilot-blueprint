...
> **設計哲學**：範例以物理本質為核心，跨產業泛化。每個標準解的價值在於它揭示的**物理機制**，不在於特定產品的應用。
...
- **S1（物質1）**：被作用的對象（Object）— 注意其關鍵物性（密度、導熱率、楊氏模量等）
- **S2（物質2）**：作用工具（Tool）— 注意其與 S1 的物理匹配性
- **F（場）**：S2 對 S1 施加作用所需的能量/場（機械場、熱場、電場、磁場、化學場等）— 注意其遵循的控制方程式

模型狀態分類：
| 狀態 | 說明 | 對應物理現象 |
|------|------|------------|
| **不完整系統** | 缺少 S1、S2 或 F 中的某個元素 | 能量傳遞路徑斷開 |
| **有效完整系統** | 三要素齊全且功能正常 | 能量從 S2 經 F 正確傳遞至 S1 |
| **有害完整系統** | 三要素齊全但產生有害效應 | 能量傳遞產生非預期副作用（如散熱路徑也傳導振動） |
| **不足效應系統** | 三要素齊全但作用效果不足 | 場的強度/密度/梯度不夠（如 flux density 太低） |

76 標準解的核心邏輯：根據 Su-Field 模型的狀態，從五大類標準解中匹配最適合的改進方向。匹配時應從 **物理根因** 出發，不從產品類比出發。
...
> **應用範例（泛化）**：**物理本質**：帕斯卡定律 — 密閉流體傳遞壓力。不限於煞車：任何需要遠端力傳遞的系統（液壓手術機器人、飛機操縱桿）都用相同 Su-Field 補全策略。**跨域**：達文西手術機器人的液壓觸覺反饋系統（S1=手術刀具, S2=操縱桿, F=液壓場）。
...
> **應用範例（泛化）**：電池芯內部添加導熱填料（添加物），使電池模組（S1）與散熱結構（S2）之間的熱場（F）傳遞更完整。
...
> **應用範例（泛化）**：**物理本質**：振動阻抗不匹配 — S3 的聲阻抗與 S1/S2 差異越大，隔離效果越好。**泛化**：任何「有害機械場傳遞」都可用此模式。**跨域**：半導體光刻機用空氣彈簧（S3, 阻抗近零）隔離地面微振，精度達 nm 等級。航太渦輪引擎用黏彈性阻尼環隔離葉片共振。
...
> **應用範例（泛化）**：鏈條傳動噪音問題，將金屬鏈條（S2）改為碳纖維皮帶（S2'），消除金屬摩擦噪音。
...
> **應用範例（泛化）**：**物理本質**：疊加原理 — 兩個等幅反相場相加為零。**泛化**：主動消噪耳機（聲場 F'）、振動主動抑制（機械場 F'）、電力系統有源濾波器（電磁場 F'）都是同一個 Su-Field 模式。**跨域**：MRI 掃描室用主動屏蔽線圈產生反向磁場，將雜散場衰減 100 倍。

#### 1.2.4 利用磁場與磁性物質消除有害作用（Use Magnetic Field and Ferromagnetic Substance）
**利用磁場結合鐵磁物質來隔離或消除有害效應**

> **應用範例（泛化）**：速度感測器受外部磁場干擾產生誤讀，加入鐵磁遮罩層包覆感測器，引導雜散磁場繞過。
...
> **應用範例（泛化）**：踏板力矩（F1）→ 力矩感測器（S2）→ 電子信號（F2）→ 控制器（S3）→ PWM 驅動（F3）→ 馬達（S4），形成鏈式 Su-Field。
...
> **應用範例（泛化）**：煞車系統同時使用機械場（碟煞）和電磁場（再生煞車），雙重制動提升效能。
...
> **應用範例（泛化）**：利用行駛風流（外部氣流場）作為電池散熱的輔助通道，無需額外風扇。
...
> **應用範例（泛化）**：利用雨水（環境物質）沖洗車架上的泥沙，設計引流槽將雨水導向關鍵清潔部位。
...
> **應用範例（泛化）**：在軸承中加入微量奈米級潤滑劑，即可大幅降低摩擦損耗。
...
> **應用範例（泛化）**：車架焊接時在管內充入大量保護氣體（Ar）防止氧化，焊接完成後氣體自然排出。
...
> **應用範例（泛化）**：以超音波場（F）清潔電路板，替代傳統化學清洗溶劑（S）。
...
> **應用範例（泛化）**：單馬達驅動改為前後雙馬達（雙系統），提升全輪驅動能力與扭矩分配。
...
> **應用範例（泛化）**：鋰電池（高能量密度）+ 超級電容（高功率密度）組成混合儲能系統，兼顧續航與瞬間大電流輸出。

#### 2.1.3 建立反向的雙系統（Create Inverse Bi-System）
**由兩個性質相反的子系統組合**

> **應用範例（泛化）**：加熱 + 冷卻的雙向溫控系統，低溫時預熱電池、高溫時主動冷卻，維持最佳工作溫度。

#### 2.1.4 組合不相容的特性（Combine Incompatible Properties）
**在同一系統中實現看似矛盾的特性**

> **應用範例（泛化）**：**物理本質**：複合材料的各向異性 — 纖維方向決定該方向的楊氏模量。**泛化**：任何需要「同一部件不同區域不同力學特性」的場景。**跨域**：飛機機翼蒙皮用不同碳纖維鋪層角度，翼根區剛性 120 GPa（承載），翼尖區柔性 40 GPa（氣彈減振）。人工膝關節用梯度多孔鈦合金，表面多孔促進骨整合、核心緻密承載。

#### 2.1.5 增加系統元素間的差異性（Increase Difference Between Elements）
**增大多系統中各元素的差異以產生新功能**

> **應用範例（泛化）**：多段變速齒輪組中，齒數比差異加大，使低速爬坡與高速巡航的切換更加明顯有效。
...
> **應用範例（泛化）**：將分離的馬達、減速齒輪箱、力矩感測器整合為一體化中置馬達單元。
...
> **應用範例（泛化）**：將機械式調速（摩擦離合器）替換為電子調速（PWM 控制器），提升精確度。

#### 2.2.2 將物質碎片化（Fragment S2）
**將工具物質 S2 分解為更細小的顆粒或碎片**

> **應用範例（泛化）**：將整塊散熱片改為微通道散熱鰭片陣列，增大散熱面積。

#### 2.2.3 使用毛細管或多孔結構（Use Capillary / Porous Structures）
**引入毛細管或多孔材料增強物質傳輸**

> **應用範例（泛化）**：在電池液冷板中採用微孔燒結結構，利用毛細作用均勻分配冷卻液。

#### 2.2.4 增加系統動態性（Increase Dynamism）
**使系統從剛性轉為可變形、可調節**

> **應用範例（泛化）**：可調節角度的車把手立管，騎行者可根據路況即時調整騎姿。

#### 2.2.5 結構化場（Structure the Field）
**將均勻場改為非均勻（結構化）場以提高效能**

> **應用範例（泛化）**：馬達定子繞組採用集中繞組（非均勻磁場分佈），在特定位置集中磁通量提升效率。

#### 2.2.6 結構化物質（Structure the Substance）
**將均質物質改為非均質結構以提高效能**

> **應用範例（泛化）**：車架管材從等壁厚改為變壁厚（受力處加厚、非受力處減薄），減重同時維持強度。
...
> **應用範例（泛化）**：PWM 驅動頻率避開馬達機械共振頻率，避免產生振動噪音。

#### 2.3.2 場與場之間的節奏匹配（Match Rhythms of Two Fields）
**協調兩個場之間的節奏關係**

> **應用範例（泛化）**：電池充電脈衝頻率與電池內部化學反應速率匹配，提高充電效率減少衰減。

#### 2.3.3 利用兩個不同場（Use Two Incompatible Fields）
**同時施加兩個不同頻率/類型的場，利用其交互作用**

> **應用範例（泛化）**：馬達同時接受高頻 PWM 電場驅動與低頻機械振動反饋，實現自適應控制。
...
> **應用範例（泛化）**：將 eBike 系統與智慧城市交通管理系統整合，實現即時路況導航與車群協調。

#### 3.1.2 利用超系統中的資源（Use Super-System Resources）
**從超系統中提取可用資源以改善子系統**

> **應用範例（泛化）**：利用手機 GPS 與加速度感測器（超系統資源），替代 eBike 自身的獨立感測器，降低成本。

#### 3.1.3 在超系統層級消除有害因素（Eliminate Harmful Factors at Super-System Level）
**把子系統無法解決的問題，上升到超系統層級處理**

> **應用範例（泛化）**：單車防盜在車輛層級難以完善，上升至社區共享停車站層級設計統一監控防盜系統。
...
> **應用範例（泛化）**：傳統機械接點式開關改為半導體 MOSFET 固態開關，從巨觀接觸轉為微觀電子層級控制。

#### 3.2.2 利用微觀結構（Use Micro-Structures）
**在微觀層級設計結構以實現巨觀功能**

> **應用範例（泛化）**：車架表面施加奈米疏水塗層（微觀結構），實現巨觀的自潔防鏽功能。

#### 3.2.3 利用物質在微觀層級的物理/化學特性（Use Micro-Level Physical/Chemical Properties）
**利用物質在分子或原子層級的特殊性質**

> **應用範例（泛化）**：鋰電池正極材料採用奈米級磷酸鐵鋰顆粒，利用其微觀層級的高表面積特性提升充放電速率。
...
> **應用範例（泛化）**：無法直接測量電池內部溫度，透過量測電池內阻變化（間接參數）推算內部核心溫度。
...
> **應用範例（泛化）**：在馬達控制中建構轉速測量 Su-Field：磁鐵（S1 在轉子）+ 霍爾感測器（S2）+ 磁場（F），量測轉速。
...
> **應用範例（泛化）**：力矩感測器靈敏度不足，在應變規表面增加磁致伸縮塗層（附加場效應），放大力矩信號。
...
> **應用範例（泛化）**：在鏈條每個鏈節嵌入微型磁性標記，以磁感測器非接觸式量測鏈條伸長量（磨損程度）。
...
> **應用範例（泛化）**：利用壓電效應（物理效應）量測輪胎壓力，壓電薄膜感測器嵌入輪圈內。

#### 4.5.2 利用共振進行測量（Use Resonance for Measurement）
**利用共振頻率變化來檢測參數變化**

> **應用範例（泛化）**：車架結構健康監測——在車架上安裝壓電元件激發振動，透過共振頻率偏移檢測裂紋。
...
> **應用範例（泛化）**：馬達產生的磁場同時用於驅動轉子和為轉速感測器提供偵測信號，一場兩用。

#### 5.2.2 利用環境中已存在的場（Use Fields from Environment）
**利用環境中自然存在的場作為功能資源**

> **應用範例（泛化）**：利用地球重力場，設計再生煞車系統在下坡時自動回收動能轉為電能。

#### 5.2.3 利用能夠攜帶場的物質（Use Substances that Carry Fields）
**選擇本身攜帶所需場效應的物質**

> **應用範例（泛化）**：選用永磁材料（自帶磁場）作為馬達轉子，無需外部勵磁線圈。
...
> **應用範例（泛化）**：**物理本質**：潛熱儲存 — 相變過程中溫度不變但吸收/釋放大量熱（ΔH_fus）。**泛化**：任何需要「恆溫緩衝」的系統。**跨域**：航太衛星熱控用石蠟 PCM（ΔH≈200 kJ/kg）在日照/陰影交替時穩定電子元件溫度。建築牆體嵌入微膠囊 PCM 降低空調負荷 20-30%。醫療器械用冰漿灌注實現精確低溫手術。

#### 5.3.2 利用「雙態」物質（Use Dual-State Substance）
**同一物質在兩種狀態間切換以實現不同功能**

> **應用範例（泛化）**：形狀記憶合金製成的通風百葉窗，低溫時關閉保溫、高溫時自動張開通風（固態↔回復態）。

#### 5.3.3 利用相變產生的伴隨效應（Use Accompanying Effects of Phase Transition）
**利用相變過程中產生的體積變化、熱量釋放/吸收等效應**

> **應用範例（泛化）**：水結冰膨脹的原理用於防凍監測——在暴露管路中設置膨脹感測器，冰點前發出預警。

#### 5.3.4 利用兩相物質替代單相（Replace Single-Phase with Two-Phase Substance）
**用雙相混合物替代單相物質以獲得額外特性**

> **應用範例（泛化）**：散熱管內使用氣液兩相工質（熱管），相變過程高效傳熱遠優於純液態冷卻液。

#### 5.3.5 利用相互作用的物質（Use Interacting Substances / Physicochemical Effects）
**利用不同物質之間的物理或化學交互作用**

> **應用範例（泛化）**：自修復輪胎——橡膠內層含微膠囊，刺穿時膠囊破裂釋放黏合劑（化學反應），自動封堵孔洞。
...
> **應用範例（泛化）**：電致變色車燈罩，根據環境光照強度自動調節透光率，白天高透光、夜間降低眩光。

#### 5.4.2 利用多物理效應耦合（Use Coupled Physical Effects）
**利用不同物理效應之間的耦合關係**

> **應用範例（泛化）**：壓電-電磁耦合式能量收集器，同時從振動中透過壓電效應和電磁感應收集能量。
