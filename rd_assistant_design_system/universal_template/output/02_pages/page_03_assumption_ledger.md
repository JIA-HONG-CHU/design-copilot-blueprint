...
- **secondary_goal**: 建立假設 ↔ 矛盾 ↔ 收斂圖的完整可追溯鏈。部分假設可由 AI 自動從上傳的多模態素材中提取 (PRD F1.7)，標記來源以利追溯。索克拉底七類提問中的「因果追問」與「假設挑戰」結果自動回饋至本頁。
...
- **elements**:
  ...
  - `索克拉底回饋`: `feedback-list`, `optional`, `顯示來自 Step 2 七類提問中「因果追問」和「假設挑戰」的 AI 回饋結論，作為假設驗證的輔助依據。`
- **states**:
  - collapsed：未選中假設時收合。
...
- [x] 索克拉底七類提問中「因果追問」和「假設挑戰」的 AI 回饋能正確顯示在追溯面板。
