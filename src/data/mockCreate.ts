import type {
  AntiAnchorRoute, TrizSolution, Subsystem, ScamperVariant, Alternative
} from "@/types/create";

export const mockAntiAnchorRoutes: Record<string, AntiAnchorRoute[]> = {
  "proj-001": [
    { id: "aar-001", name: "直驅輪轂方案", description: "完全捨棄傳統中驅+傳動系統，改用輪轂馬達直接驅動後輪，消除傳動效率損失與噪音來源。" },
  ],
};
...
export const mockSubsystems: Record<string, Subsystem[]> = {
  "proj-001": [
    { id: "ss-001", name: "動力傳動模組", reason: "與馬達轉速、傳動效率直接相關", relatedContradictions: ["ec-001"], confirmed: true },
    { id: "ss-002", name: "殼體結構模組", reason: "與材料強度、重量直接相關", relatedContradictions: ["ec-002"], confirmed: true },
    { id: "ss-003", name: "散熱模組", reason: "馬達轉速提高將增加散熱需求", relatedContradictions: ["ec-001"], confirmed: false },
    { id: "ss-004", name: "密封模組", reason: "IP55 防護等級涉及密封設計", relatedContradictions: [], confirmed: false },
  ],
};
  ...
  "proj-001": [
    {
      ...
      mustScores: { M1: "pass", M2: "pass", M3: "pass", M4: "pass", M5: "marginal" },
      preCadScores: { space: 4, cost: 3, safety: 4, decoupling: 5, supply: 3 },
      overallPass: true,
    },
    {
      id: "alt-002",
      ...
      mustScores: { M1: "pass", M2: "pass", M3: "marginal", M4: "pass", M5: "pass" },
      preCadScores: { space: 3, cost: 4, safety: 3, decoupling: 3, supply: 4 },
      overallPass: true,
    },
    {
      id: "alt-003",
      ...
      mustScores: { M1: "pass", M2: "fail", M3: "pass", M4: "pass", M5: "fail" },
...
export const mockAntiAnchorWarning: Record<string, string> = {
  "proj-001": "⚠ 路徑依賴風險偵測：目前專案以「皮帶傳動 + 中驅馬達」為基準設計路線。團隊過去 3 個專案皆採用類似架構，可能存在路徑依賴偏差。建議至少探索 1 條完全不同的技術路線（如直驅、磁力耦合、液壓傳動等）。",
};
