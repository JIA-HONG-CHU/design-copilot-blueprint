/**
 * Seed data utilities for development — 3 showcase projects covering full TRIZ lifecycle.
 *
 * Uses service_role admin client to bypass RLS.
 * All 28 Supabase tables are populated for Project 1 (completed).
 */

import { createClient } from '@supabase/supabase-js';

// ---------------------------------------------------------------------------
// Admin client (lazy init — bypasses RLS)
// ---------------------------------------------------------------------------
...
function getAdminDb() {
  if (!_adminDb) {
    ...
    if (!key) throw new Error('[seed] VITE_SUPABASE_SERVICE_ROLE_KEY not set in .env');
    _adminDb = createClient(url, key, { auth: { persistSession: false, autoRefreshToken: false } });
  }
  return _adminDb;
}
...
async function ins(table: string, data: Record<string, unknown> | Record<string, unknown>[], label: string) {
  const { error } = await getAdminDb().from(table).insert(data as any);
  if (error) throw new Error(`[seed] ${label}: ${error.message}`);
}

async function insRet<T = { id: string }>(table: string, data: Record<string, unknown> | Record<string, unknown>[], label: string): Promise<T[]> {
  const { data: rows, error } = await getAdminDb().from(table).insert(data as any).select();
  if (error) throw new Error(`[seed] ${label}: ${error.message}`);
  return rows as T[];
}

/** Get the currently logged-in user ID (for created_by / user_id fields) */
async function getUserId(): Promise<string> {
  // Try real auth first
  const { data } = await getAdminDb().auth.getUser();
  if (data?.user) return data.user.id;
  // Fallback: get first user from admin API
  const { data: list } = await getAdminDb().auth.admin.listUsers({ page: 1, perPage: 1 });
  if (list?.users?.[0]) return list.users[0].id;
...
async function seedProject1(uid: string): Promise<string> {
  ...
  await ins('briefs', {
    ...
    task_definition_5w1h: {
      who: 'Delta Electronics 電力電子事業部',
      what: '馬達控制器 PCB 與韌體重新設計',
      where: 'E-bike 動力傳動系統整合',
      when: '2026 Q3 量產目標',
      why: '現行版本在持續爬坡場景超出散熱預算',
      how: '拓撲最佳化 + GaN MOSFET 評估 + 熱模擬',
    },
  }, 'brief-1');
  ...
  const alts = await insRet('alternatives', [
    {
      ...
      must_scores: {
        ...
        description: 'GaN 方案：效率 97.2%、散熱降 40%、通過 CISPR 25 Class 5 EMI 認證',
        assumptions: ['GaN 元件可靠性滿足車規 AEC-Q101', 'IMS 鋁基板供應鏈穩定'],
        mustCriteria: [
          { id: 'M1', label: '效率 ≥ 95%', passed: true },
          { id: 'M2', label: '散熱餘裕 ≥ 20%', passed: true },
          { id: 'M3', label: 'CISPR 25 Class 5', passed: true },
          { id: 'M4', label: '成本 ≤ $12', passed: true },
          { id: 'M5', label: '可製造性 (SMT 相容)', passed: true },
          { id: 'M6', label: 'ISO 26262 ASIL-B', passed: true },
        ],
        risks: [{ id: 'R1', description: 'GaN 供應商單一來源風險', severity: 'major', mitigation: '備選 EPC 2nd source 驗證中' }],
        minValidation: '原型板 thermal cycling 測試 500 次 + EMI 掃描',
      },
    ...
    },
    {
      ...
      must_scores: {
        M1: 'fail', M2: 'pass', M3: 'pass', M4: 'pass', M5: 'pass', M6: 'pass',
        description: 'Si 最佳化方案：效率 93.5%、散熱依賴加大面積',
        assumptions: ['4oz 銅 PCB 內層製程穩定，不影響信號完整性'],
        mustCriteria: [
          { id: 'M1', label: '效率 ≥ 95%', passed: false },
          { id: 'M2', label: '散熱餘裕 ≥ 20%', passed: true },
          { id: 'M3', label: 'CISPR 25 Class 5', passed: true },
          { id: 'M4', label: '成本 ≤ $12', passed: true },
          { id: 'M5', label: '可製造性 (SMT 相容)', passed: true },
        ...
        risks: [{ id: 'R2', description: '效率未達 95% 目標', severity: 'fatal', mitigation: '需加入 LLC 諧振輔助' }],
        minValidation: '效率量測 under full load (48V/15A)',
      },
    ...
    },
    {
      ...
      must_scores: {
        M1: 'fail', M2: 'fail', M3: 'pass', M4: 'fail', M5: 'pass', M6: 'marginal',
        description: '基線方案作為對照，未做任何優化',
        mustCriteria: [
          { id: 'M1', label: '效率 ≥ 95%', passed: false },
          { id: 'M2', label: '散熱餘裕 ≥ 20%', passed: false },
          { id: 'M3', label: 'CISPR 25 Class 5', passed: true },
          { id: 'M4', label: '成本 ≤ $12', passed: false },
          { id: 'M5', label: '可製造性 (SMT 相容)', passed: true },
          { id: 'M6', label: 'ISO 26262 ASIL-B', passed: null },
        ],
        risks: [],
        minValidation: '無',
      },
  ...
  // ── Concept Routes (composition uses CompositionEntry[] format) ──
  await ins('concept_routes', [
    {
      project_id: pid, route_type: 'composite',
      composition: [
        { solutionId: 'triz-28', sourcePrinciple: '#28 機械替代', concrete: 'GaN 諧振軟切換拓撲 (200kHz)', dimension: '拓撲', adoptionType: 'M2' },
        { solutionId: 'triz-1', sourcePrinciple: '#1 分割', concrete: 'IMS 鋁基板功率級分離', dimension: '空間', adoptionType: 'M1' },
        { solutionId: 'triz-19', sourcePrinciple: '#19 週期性動作', concrete: '展頻調變 EMI 抑制 (±10%)', dimension: '頻率', adoptionType: 'M2' },
      ],
      composition_rationale: '結合 TRIZ 原理 #28(軟切換) + #1(空間分離) + #19(展頻)，三者分別作用於拓撲/空間/頻率維度。#28+#19 形成互相強化 (M2)，#1 獨立維度 (M1)。全面解決效率/EMI/散熱三大矛盾。',
      anti_pattern_warnings: [],
    },
    {
      project_id: pid, route_type: 'single',
      composition: [
        { solutionId: 'triz-35', sourcePrinciple: '#35 參數變更', concrete: '低 Qg Si MOSFET + 4oz 銅面 + 加大散熱片', dimension: '參數最佳化', adoptionType: 'M4' },
      ],
      composition_rationale: '保守方案，不改變拓撲僅最佳化參數。與 GaN 路線物理互斥 (M4)，作為獨立 Route。',
      anti_pattern_warnings: ['與現有方案差異小，可能陷入路徑依賴'],
    },
  ], 'concept-routes-1');

  // ── Compatibility Pairs (adoption_type uses M1-M5 format) ──
  ...
  await ins('evidence_entries', [
    {
      project_id: pid, user_id: uid, title: 'Dyno 效率測試結果',
      measured_value: '97.2', unit: '%', evidence_level: 'E3',
      method: '48V Dyno 測試', notes: '實測效率超越目標 2.2%',
      measured_at: '2026-02-20T10:00:00Z',
      kpi_id: kpiIds1[0], linked_assumption_codes: ['A-01', 'A-04'],
      linked_must_ids: ['M1'],
    },
    {
      project_id: pid, user_id: uid, title: 'IR 熱成像散熱測試',
      measured_value: '42', unit: '% 降低', evidence_level: 'E3',
      method: '500W 持續負載 IR 熱成像', notes: '散熱降幅超過目標 12%',
      measured_at: '2026-02-22T14:00:00Z',
      kpi_id: kpiIds1[1], linked_assumption_codes: ['A-01'],
      linked_must_ids: ['M2'],
    },
    {
      project_id: pid, user_id: uid, title: 'BOM 成本核算',
      measured_value: '11.80', unit: 'USD', evidence_level: 'E3',
      method: '10k 量級 BOM rollup', notes: '含 GaN 模組，低於 $12 目標',
      measured_at: '2026-02-25T09:00:00Z',
      kpi_id: kpiIds1[2], linked_assumption_codes: ['A-02'],
      linked_must_ids: ['M4'],
    },
  ], 'evidence-entries-1');
  ...
  const [decision] = await insRet('decisions', {
    project_id: pid,
    selected_alternative_id: alts[0].id,
    selected_alternative_name: 'GaN 軟切換 + IMS 分離方案',
  ...
  const wantScores: Record<string, unknown>[] = [];
  crids.forEach((cid, i) => {
  ...
  console.info(`[seed] Project 1 (Motor Controller) created: ${pid}`);
  return pid;
}
...
async function seedProject2(uid: string): Promise<string> {
  ...
  await ins('briefs', {
    ...
    mission: '設計新一代 48V/20Ah 鋰電池 BMS，實現 ≥98% 均衡效率且滿足 UN38.3 安全認證。',
    task_definition_5w1h: {
      who: 'Delta Electronics 儲能事業部',
      what: 'BMS PCB + 韌體 + 均衡演算法開發',
      where: '高階 E-bike 電池包整合',
      when: '2026 Q4 量產',
      why: '現行被動均衡效率僅 70%，無法支撐長距離騎乘',
      how: '主動均衡拓撲 + SoC/SoH 估測演算法 + 安全認證規劃',
    },
  }, 'brief-2');
  ...
  await ins('cld_edges', [
    { project_id: pid, from_node: nids[0], to_node: nids[1], polarity: '+' },
    { project_id: pid, from_node: nids[1], to_node: nids[2], polarity: '+' },
    { project_id: pid, from_node: nids[0], to_node: nids[3], polarity: '+' },
  ...
  await ins('triz_solutions', [
    ...
    { project_id: pid, contradiction_id: cids[1], path: 'TC', principle_number: 24, principle_name: '中介物', suggestion: '使用共用 ADC + 多工器替代獨立感測器，降低成本保持精度。', status: 'adopted' },
  ], 'triz-2');

  const subsys = await insRet('subsystems', [
    { project_id: pid, name: '均衡電路', reason: '主動均衡拓撲選擇影響效率與面積', related_contradictions: [cids[0]], confirmed: true, source: 'rd' },
    { project_id: pid, name: 'SoC/SoH 估測', reason: '演算法精度影響續航預估與安全判斷', related_contradictions: [cids[1]], confirmed: true, source: 'rd' },
  ...
  const alts2 = await insRet('alternatives', [
    {
      ...
      must_scores: {
        M1: 'pass', M2: 'pass', M3: 'marginal', M4: 'pass', M5: 'pass', M6: 'pass',
        description: 'Flyback 3D 堆疊方案：均衡效率 98.2%，面積不增',
        assumptions: ['Flyback 變壓器可微型化至 6×6mm', '軟切換 EMI 可控'],
        mustCriteria: [
          { id: 'M1', label: '均衡效率 ≥ 95%', passed: true },
          { id: 'M2', label: '充放電循環 ≥ 2000', passed: true },
          { id: 'M3', label: 'EMI 不干擾 CAN', passed: null },
          { id: 'M4', label: 'BOM ≤ $8', passed: true },
          { id: 'M5', label: '可量產性', passed: true },
          { id: 'M6', label: 'UN38.3 認證', passed: true },
        ],
        risks: [{ id: 'R1', description: 'PoP 堆疊焊接良率', severity: 'major', mitigation: 'X-ray AOI 全檢' }],
        minValidation: 'PoP 焊接良率驗證 + EMI 掃描',
      },
    ...
    },
    {
      ...
      must_scores: {
        M1: 'pass', M2: 'pass', M3: 'pass', M4: 'pass', M5: 'pass', M6: 'pass',
        description: '電感型均衡：效率 95.5%，面積小但速度慢',
        assumptions: ['共用電感切換速度滿足需求'],
        mustCriteria: [
          { id: 'M1', label: '均衡效率 ≥ 95%', passed: true },
          { id: 'M2', label: '充放電循環 ≥ 2000', passed: true },
          { id: 'M3', label: 'EMI 不干擾 CAN', passed: true },
          { id: 'M4', label: 'BOM ≤ $8', passed: true },
          { id: 'M5', label: '可量產性', passed: true },
          { id: 'M6', label: 'UN38.3 認證', passed: true },
        ],
        risks: [],
        minValidation: '均衡速度量測 (20S 全串完成)',
      },
  ...
  (kpiRows2 || []).forEach((k: any) => { kpiMap2[k.kpi_name] = k.id; });

  await ins('evidence_entries', [
    {
      project_id: pid, user_id: uid, title: '均衡 EMI 預掃描結果',
      measured_value: '< 10^-8', unit: 'BER', evidence_level: 'E2',
      method: 'CAN bus BER 測試 (軟切換模式)', notes: '軟切換後 BER 遠低於 10^-6 門檻',
      measured_at: '2026-03-05T14:00:00Z',
      kpi_id: kpiMap2['均衡效率'] || null, linked_assumption_codes: ['A-03'],
      linked_must_ids: ['M3'],
    },
    {
      project_id: pid, user_id: uid, title: '待機功耗量測',
      measured_value: '38', unit: 'μA', evidence_level: 'E3',
      method: '功率分析儀 24hr 監測', notes: '含 RTC 保活，低於 50μA 目標',
      measured_at: '2026-03-08T10:00:00Z',
      kpi_id: kpiMap2['待機功耗'] || null, linked_assumption_codes: ['A-04'],
      linked_must_ids: ['M5'],
    },
    {
      project_id: pid, user_id: uid, title: '共用 ADC 精度驗證',
      measured_value: '2.8', unit: '% 誤差', evidence_level: 'E2',
      method: '全溫度範圍 SoC 估測對比庫倫計數', notes: '25°C 精度 1.5%，-20°C 精度 4.2%（需改善）',
      measured_at: '2026-03-10T09:00:00Z',
      kpi_id: kpiMap2['SoC 估測精度'] || null, linked_assumption_codes: ['A-02'],
      linked_must_ids: ['M1'],
    },
  ], 'evidence-entries-2');
  ...
  await ins('concept_routes', [
    {
      project_id: pid, route_type: 'composite',
      composition: [
        { solutionId: 'triz-17', sourcePrinciple: '#17 另一維度', concrete: 'Flyback 變壓器 3D PoP 垂直堆疊', dimension: '空間', adoptionType: 'M1' },
        { solutionId: 'triz-24', sourcePrinciple: '#24 中介物', concrete: '共用 ADC + 多工器替代獨立感測器', dimension: '感測', adoptionType: 'M2' },
      ],
      composition_rationale: '結合 TRIZ #17(3D 堆疊解決面積) + #24(共用 ADC 降成本)，兩者作用於不同維度(空間/感測)，互不衝突(M1+M2)。',
      anti_pattern_warnings: [],
    },
    {
      project_id: pid, route_type: 'single',
      composition: [
        { solutionId: 'triz-6', sourcePrinciple: '#6 萬用性', concrete: '共用變壓器 + 多工切換均衡', dimension: '拓撲', adoptionType: 'M4' },
      ],
      composition_rationale: '單一共用變壓器方案，與 Flyback 獨立變壓器路線物理互斥(M4)。面積最省但效率略低。',
      anti_pattern_warnings: ['共用變壓器切換速度可能限制均衡效能'],
    },
  ], 'concept-routes-2');

  // ── Compatibility Pairs (Create 階段相容性分析) ──
  await ins('compatibility_pairs', [
    { project_id: pid, solution_a_id: alts2[0].id, solution_b_id: alts2[1].id, result: 'exclusive', adoption_type: 'M4', reason: '兩方案均衡拓撲互斥：Flyback 獨立變壓器 vs 共用電感切換，無法在同一均衡電路共存。SoC/SoH 估測子系統可獨立採用(M1)。' },
  ], 'compat-2');

  console.info(`[seed] Project 2 (BMS) created: ${pid}`);
  return pid;
}
...
async function seedProject3(uid: string): Promise<string> {
  ...
  await ins('briefs', {
    ...
    mission: '將車架重量從 2.8kg 降至 2.0kg 以下，同時維持 ISO 4210 剛性與疲勞測試標準。',
    task_definition_5w1h: {
      who: 'Delta Mobility 結構設計部',
      what: '車架管材選型、幾何最佳化、接合工藝開發',
      where: '公路型 E-bike',
      when: '2027 Q1 試產',
      why: '市場調查顯示重量是消費者購買決策第二大因素',
      how: '拓撲最佳化 + 碳纖維/鋁合金複合材料評估 + FEA 模擬',
    },
  }, 'brief-3');
  ...
  await ins('cld_edges', [
    { project_id: pid, from_node: nids[0], to_node: nids[1], polarity: '+' },
    { project_id: pid, from_node: nids[0], to_node: nids[2], polarity: '+' },
    { project_id: pid, from_node: nids[4], to_node: nids[1], polarity: '-' },
    { project_id: pid, from_node: nids[4], to_node: nids[3], polarity: '+' },
...
export async function seedShowcaseProjects(): Promise<string[]> {
  ...
  console.info(`[seed] All 3 showcase projects created: ${ids.join(', ')}`);
  return ids;
}

export async function clearDemoData(projectId: string): Promise<void> {
  if (!projectId) throw new Error('projectId is required');
  const { error } = await getAdminDb().from('projects').delete().eq('id', projectId);
...
export async function clearAllShowcaseData(): Promise<void> {
  ...
  console.info('[seed] All showcase data cleared.');
}
