...
async function seedProject2(uid: string): Promise<string> {
  const [project] = await insRet('projects', {
    ...
    quick_stats: { contradictions_count: 2, assumptions_count: 4, alternatives_count: 2, risks_count: 3, experiments_count: 2, evidence_items_count: 4 },
    must_criteria_config: [
...
async function seedProject3(uid: string): Promise<string> {
  const [project] = await insRet('projects', {
    ...
    quick_stats: { contradictions_count: 2, assumptions_count: 0, alternatives_count: 0, risks_count: 0, experiments_count: 0, evidence_items_count: 0 },
    must_criteria_config: [
  ...
  // P3 is Phase I only — no triz_solutions, subsystems, or assumptions yet
